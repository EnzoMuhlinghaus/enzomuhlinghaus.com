...
const ZONE_NAMES: Record<string, Record<string, string>> = {
  en: { easy: 'Easy', marathon: 'Marathon', threshold: 'Threshold', vo2max: 'VO2max' },
  fr: { easy: 'Endurance', marathon: 'Marathon', threshold: 'Seuil', vo2max: 'VO2max' },
};
...
const zones = computed(() => {
  ...
  const names = ZONE_NAMES[fr.value ? 'fr' : 'en']!;
  return ZONE_PCTS.map((z) => ({
    key: z.key,
    name: names[z.key]!,
    pct: `${Math.round(z.pct * 100)}% ${fr.value ? 'VMA' : 'MAS'}`,
...
<template>
  ...
  <div class="head-row">
    <div class="mono-label">{{ fr ? '→ ESTIMÉ' : '→ ESTIMATED' }}</div>
    <div class="unit-row">
      <button
  ...
  <div class="cards">
    ...
    <div class="dashed-panel card">
      <div class="card-title">{{ fr ? 'VMA' : 'MAS' }}</div>
      <div class="card-value">{{ masKmh != null ? `${masKmh.toFixed(1)} km/h` : '—' }}</div>
      <div class="card-sub">{{ masKmh != null ? speedToPaceStr(masKmh) : '' }}</div>
    </div>
  </div>

  <div class="mono-label section-label">{{ fr ? "ALLURES D'ENTRAÎNEMENT" : 'TRAINING PACES' }}</div>
